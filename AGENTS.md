...
`live_ceci` is a plain Elixir OTP app: Bandit + Plug.Router + a raw `WebSock` handler
...
- **P2** Blocking calls in a `WebSock` callback — `handle_in/2` runs on the socket
  ...
  and a `catch :exit`; see `LiveCeci.LiveSession`.
...
- `lib/live_ceci/tools.ex` calls `String.to_atom/1` inside `arg/2`. The key is always a
...
- `lib/live_ceci/socket.ex` calls `Session.start_link/1` outside a supervision tree.
